...
export const SalesKanbanCard: React.FC<SalesKanbanCardProps> = ({ item, stage }) => {
  ...
  return (
    <>
      <div className="bg-white/95 backdrop-blur-sm rounded-lg border border-neutral-200/50 shadow-soft hover:shadow-soft-lg hover:border-b3x-lime-200/50 transition-all duration-200 p-2 w-full">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 mb-2">
          <div className="min-w-0 flex-1">
            <h4 className="font-semibold text-b3x-navy-900 text-[11px] flex items-center">
              {isPenBased ? (
                <>
                  <Home className="w-2.5 h-2.5 mr-1 text-info-600" />
                  Curral {item.penNumber || item.id}
                </>
              ) : (
                <>
                  <MapPin className="w-2.5 h-2.5 mr-1 text-success-600" />
                  Lote {item.lotNumber}
                </>
              )}
            </h4>
          ...
          {/* Quantity badge */}
          <span className="text-[10px] font-medium text-success-700 bg-gradient-to-r from-success-100 to-success-50 px-1.5 py-0.5 rounded-full border border-success-200 flex-shrink-0 self-start">
            {item.currentQuantity} animais
          </span>
        </div>
        ...
        <div className="space-y-1 mb-2">
          ...
          {stage === 'next_slaughter' && (
            <>
              <div className="flex items-center text-[10px] text-neutral-600">
                <Scale className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
                <span className="truncate">Peso est.: {(item.estimatedWeight / item.currentQuantity).toFixed(0)} kg/animal</span>
              </div>
              ...
              {isPenBased && item.lotsInPen && (
                <div className="flex items-center text-[10px] text-neutral-600">
                  <Users className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
          ...
          {stage === 'shipped' && (
            <>
              <div className="flex items-center text-[10px] text-neutral-600">
                <Scale className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
                <span className="truncate">Peso saída: {(item.exitWeight / item.currentQuantity).toFixed(0)} kg/animal</span>
              </div>
              
              <div className="flex items-center text-[10px] text-neutral-600">
                <Truck className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
          ...
          {stage === 'slaughtered' && (
            <>
              <div className="flex items-center text-[10px] text-neutral-600">
                <Scale className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
                <span className="truncate">Peso abate: {(item.slaughterWeight / item.currentQuantity).toFixed(0)} kg/animal</span>
              </div>
              
              <div className="flex items-center text-[10px] text-neutral-600">
                <FileText className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
                <span className="truncate">NF: {item.invoiceNumber}</span>
              </div>
              
              <div className="flex items-center text-[10px] text-neutral-600">
                <Clock className="w-2.5 h-2.5 mr-1 text-neutral-400 flex-shrink-0" />
          ...
          {stage === 'reconciled' && (
            <>
              ...
              <div className="flex items-center text-[10px] text-neutral-600">
                <CheckCircle className="w-2.5 h-2.5 mr-1 text-success-500 flex-shrink-0" />
        ...
        <div className="bg-neutral-50/80 rounded-lg p-1.5 mb-2">
        ...
        <div className="flex items-center space-x-1.5">
          ...
          {stage !== 'reconciled' && (
            <button
              ...
              className="flex-1 flex items-center justify-center space-x-0.5 py-0.5 px-1.5 text-[10px] font-medium rounded-md transition-all duration-200 shadow-soft hover:shadow-soft-lg bg-gradient-to-r from-b3x-lime-500 to-b3x-lime-600 text-b3x-navy-900 hover:from-b3x-lime-600 hover:to-b3x-lime-700"
            >
              {stage === 'next_slaughter' && (
                <>
