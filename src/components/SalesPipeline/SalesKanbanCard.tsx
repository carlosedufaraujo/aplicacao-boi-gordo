...
export const SalesKanbanCard: React.FC<SalesKanbanCardProps> = ({ item, stage }) => {
  ...
  return (
    <>
      <div className="bg-white/95 backdrop-blur-sm rounded-lg border border-neutral-200/50 shadow-soft hover:shadow-soft-lg hover:border-b3x-lime-200/50 transition-all duration-200 p-3">
        {/* Header */}
        <div className="flex items-start justify-between mb-3">
          <div className="min-w-0 flex-1">
            <h4 className="font-semibold text-b3x-navy-900 text-sm flex items-center">
              {isPenBased ? (
                <>
                  <Home className="w-3 h-3 mr-1 text-info-600" />
                  Curral {item.penNumber || item.id}
                </>
              ) : (
                <>
                  <MapPin className="w-3 h-3 mr-1 text-success-600" />
                  Lote {item.lotNumber}
                </>
              )}
            </h4>
          ...
          {/* Quantity badge */}
          <span className="text-xs font-medium text-success-700 bg-gradient-to-r from-success-100 to-success-50 px-2 py-1 rounded-full border border-success-200 flex-shrink-0 ml-2">
        ...
        <div className="space-y-2 mb-3">
          ...
          {stage === 'next_slaughter' && (
            <>
              <div className="flex items-center text-xs text-neutral-600">
                <Scale className="w-3 h-3 mr-2 text-neutral-400 flex-shrink-0" />
              ...
              {isPenBased && item.lotsInPen && (
                <div className="flex items-center text-xs text-neutral-600">
                  <Users className="w-3 h-3 mr-2 text-neutral-400 flex-shrink-0" />
          ...
          {stage === 'shipped' && (
            <>
              ...
              <div className="flex items-center text-xs text-neutral-600">
                <Truck className="w-3 h-3 mr-2 text-neutral-400 flex-shrink-0" />
          ...
          {stage === 'slaughtered' && (
            <>
              ...
              <div className="flex items-center text-xs text-neutral-600">
                <FileText className="w-3 h-3 mr-2 text-neutral-400 flex-shrink-0" />
                <span className="truncate">NF: {item.invoiceNumber}</span>
              </div>
              
              <div className="flex items-center text-xs text-neutral-600">
                <Clock className="w-3 h-3 mr-2 text-neutral-400 flex-shrink-0" />
          ...
          {stage === 'reconciled' && (
            <>
              ...
              <div className="flex items-center text-xs text-neutral-600">
                <CheckCircle className="w-3 h-3 mr-2 text-success-500 flex-shrink-0" />
        ...
        <div className="flex items-center space-x-2">
          ...
          {stage !== 'reconciled' && (
            <button
              ...
              className="flex-1 flex items-center justify-center space-x-2 py-2 px-3 text-sm font-medium rounded-lg transition-all duration-200 shadow-soft hover:shadow-soft-lg bg-gradient-to-r from-b3x-lime-500 to-b3x-lime-600 text-b3x-navy-900 hover:from-b3x-lime-600 hover:to-b3x-lime-700"
            >
              {stage === 'next_slaughter' && (
                <>
